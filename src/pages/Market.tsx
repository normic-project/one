...
import { autoMarketContract, compatible, errorMessage, eventMarketContract, loadMarket, loadOrders, loadTrades,
...
export default function MarketPage() {
  ...
  const [proposalOutcome, setProposalOutcome] = useState(1);
  const [proposalEvidence, setProposalEvidence] = useState('');
  const [disputeOutcome, setDisputeOutcome] = useState(2);
  const [disputeEvidence, setDisputeEvidence] = useState('');
  const [adjudicatedOutcome, setAdjudicatedOutcome] = useState(1);
  ...
  async function propose() {
    ...
    await tx.run(async update => {
      const signer = await approveUSDG(address, market.event!.proposalBond, update);
      update('Confirm the bonded outcome proposal…');
      return eventMarketContract(address, signer).propose(proposalOutcome, proposalEvidence);
    });
  }
  async function dispute() {
    ...
    await tx.run(async update => {
      const signer = await approveUSDG(address, market.event!.proposalBond, update);
      update('Confirm the bonded dispute…');
      return eventMarketContract(address, signer).dispute(disputeOutcome, disputeEvidence);
    });
  }
  ...
  return <div className="page"><Link className="back-link" to="/"><ArrowLeft size={15} />All markets</Link><StatusBanner />
    ...
    {!market ? <EmptyState title={protocol ? 'Loading market…' : 'No live market connected'}>Market details are read directly from verified mainnet contracts.</EmptyState> : <>
      ...
      <div className="trading-layout"><div>
        ...
        <section className="panel detail-section"><div className="section-title"><h2>Resolution rules</h2><span className="badge">{market.marketType === 0 ? 'Historical TWAP' : 'Optimistic resolution'}</span></div><p>{market.rules}</p><dl className="details"><dt>Market type</dt><dd>{market.marketType === 0 ? 'AUTO_MARKET' : 'EVENT_MARKET'}</dd><dt>Current state</dt><dd>{stateLabel(market.state)}</dd><dt>YES meaning</dt><dd>{market.yesOutcome}</dd><dt>NO meaning</dt><dd>{market.noOutcome}</dd><dt>Trading close</dt><dd>{new Date(market.closesAt * 1000).toUTCString()}</dd><dt>Resolution date</dt><dd>{new Date(market.resolvesAt * 1000).toUTCString()}</dd><dt>Primary source</dt><dd>{primaryHref ? <a href={primaryHref} target="_blank" rel="noopener noreferrer">Open source <ArrowUpRight size={13} /></a> : <span>{market.primarySource}</span>}</dd>{market.secondarySource && <><dt>Secondary source</dt><dd>{secondaryHref ? <a href={secondaryHref} target="_blank" rel="noopener noreferrer">Open backup source <ArrowUpRight size={13} /></a> : <span>{market.secondarySource}</span>}</dd></>}<dt>Metadata commitment</dt><dd>{market.metadataHash}</dd><dt>Market contract</dt><dd><a href={`${EXPLORER}/address/${address}`} target="_blank" rel="noopener noreferrer">{short(address)} ↗</a></dd></dl>
          {market.auto && <div className="notice"><ShieldCheck size={17} /><span>Asset {market.auto.asset}; condition {market.auto.condition === 0 ? 'ABOVE OR EQUAL' : 'BELOW'}; threshold {money(market.auto.threshold)} USDG; TWAP {market.auto.twapWindow / 60} minutes; pool {short(market.auto.pool)}.</span></div>}
          {market.marketType === 0 && !market.resolved && now / 1000 >= market.resolvesAt && <button className="button secondary" disabled={tx.pending || !wallet.account} onClick={() => void tx.run(async () => autoMarketContract(address, await wallet.signer()).resolve())}>Resolve from historical TWAP</button>}
        </section>
        {market.event && <section className="panel detail-section"><div className="section-title"><h2>Event resolution</h2><span className="badge">{money(market.event.proposalBond)} USDG bond · {market.event.disputePeriod / 3600}h dispute</span></div>
          {market.state === 1 && now / 1000 >= market.resolvesAt && <div className="resolution-action"><label>Proposed outcome<select aria-label="Proposed outcome" value={proposalOutcome} onChange={e => setProposalOutcome(Number(e.target.value))}><option value={1}>YES</option><option value={2}>NO</option><option value={3}>INVALID</option></select></label><label>Proposal evidence<input aria-label="Proposal evidence" maxLength={512} value={proposalEvidence} onChange={e => setProposalEvidence(e.target.value)} placeholder="https://… or ipfs://…" /></label><button className="button secondary" disabled={!wallet.account || !proposalEvidence || tx.pending} onClick={() => void propose()}>Propose outcome · {money(market.event.proposalBond)} USDG bond</button></div>}
          {market.state >= 2 && <dl className="details"><dt>Proposed outcome</dt><dd>{outcomeLabel(market.event.proposedOutcome)}</dd><dt>Proposal evidence</dt><dd>{market.event.proposalEvidence}</dd><dt>Dispute deadline</dt><dd>{new Date(market.event.disputeDeadline * 1000).toUTCString()}</dd>{market.event.disputedOutcome > 0 && <><dt>Disputed outcome</dt><dd>{outcomeLabel(market.event.disputedOutcome)}</dd><dt>Dispute evidence</dt><dd>{market.event.disputeEvidence}</dd></>}</dl>}
          {market.state === 2 && now / 1000 < market.event.disputeDeadline && <div className="resolution-action"><label>Alternative outcome<select aria-label="Alternative outcome" value={disputeOutcome} onChange={e => setDisputeOutcome(Number(e.target.value))}><option value={1}>YES</option><option value={2}>NO</option><option value={3}>INVALID</option></select></label><label>Dispute evidence<input aria-label="Dispute evidence" maxLength={512} value={disputeEvidence} onChange={e => setDisputeEvidence(e.target.value)} placeholder="https://… or ipfs://…" /></label><button className="button secondary" disabled={!wallet.account || !disputeEvidence || disputeOutcome === market.event.proposedOutcome || tx.pending} onClick={() => void dispute()}>Dispute · {money(market.event.proposalBond)} USDG bond</button></div>}
          {market.state === 2 && now / 1000 >= market.event.disputeDeadline && <button className="button secondary" disabled={!wallet.account || tx.pending} onClick={() => void tx.run(async () => eventMarketContract(address, await wallet.signer()).finalize())}>Finalize undisputed outcome</button>}
          {market.state === 3 && wallet.account.toLowerCase() === market.event.resolverMultisig.toLowerCase() && <div className="resolution-action"><label>Final outcome<select aria-label="Final outcome" value={adjudicatedOutcome} onChange={e => setAdjudicatedOutcome(Number(e.target.value))}><option value={1}>YES</option><option value={2}>NO</option><option value={3}>INVALID</option></select></label><button className="button secondary" disabled={tx.pending} onClick={() => void tx.run(async () => eventMarketContract(address, await wallet.signer()).adjudicate(adjudicatedOutcome))}>Adjudicate disputed market</button></div>}
          {market.state === 3 && <p>Redemption remains disabled until the configured resolver multisig adjudicates this dispute.</p>}
        </section>}
      ...
      </section><div className="aside-note"><ShieldCheck size={20} /><p>Every matched YES/NO pair locks exactly 1 USDG. Fees and bonds use separate balances.</p></div></aside></div>
    </>}
  </div>;
}
